<template>
  <section id="inicio" class="relative isolate overflow-hidden pb-24 pt-16 sm:pt-24">
    <img
      :src="networkHero"
      alt=""
    ...
    <div class="section-shell grid min-w-0 items-center gap-14 lg:grid-cols-2">
      <div class="fade-up min-w-0">
        ...
        <p class="mt-6 max-w-[34ch] text-base leading-relaxed text-vogel-gray/90 sm:max-w-2xl sm:text-lg">
          Desarrollamos sistemas, dashboards, automatizaciones e inteligencia artificial para que tu empresa tenga más control,
          menos tareas manuales y mejores decisiones.
        </p>

        <div class="mt-9 flex flex-wrap items-center gap-4">
          <ActionButton label="Ver soluciones" href="#soluciones" />
          <ActionButton
      ...
      <div class="fade-up-delay relative min-w-0 overflow-hidden">
        <div class="glass-panel overflow-hidden rounded-3xl p-3 shadow-glow sm:p-4">
          ...
          <div class="grid gap-3 pt-4 sm:grid-cols-2">
            <article v-for="node in nodes" :key="node.title" class="rounded-2xl border border-vogel-gray/15 bg-vogel-deep/70 p-4">
              <p class="text-xs uppercase tracking-[0.08em] text-vogel-gray/70">{{ node.title }}</p>
              <p class="mt-2 text-xl font-semibold text-white">{{ node.value }}</p>
            </article>
          </div>
...
import networkHero from "../assets/hero/network-intelligence.png";
import dashboardMockup from "../assets/hero/dashboard-mockup.png";

const nodes = [
  { title: "Procesos detectados", value: "+28" },
  { title: "Tareas automatizadas", value: "74%" },
  { title: "Visibilidad operativa", value: "Tiempo real" },
  { title: "Impacto en rentabilidad", value: "+19%" },
];
</script>
